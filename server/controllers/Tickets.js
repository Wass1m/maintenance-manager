...
var getTicketById = async (req, res) => {
  try {
    ...
    return res.json({ ticket, anomalies });
  } catch (error) {
    res.status(500).send("Erreur serveur");
  }
};
...
var getTicketByResponsable = async (req, res) => {
  try {
    var tickets = await db.sequelize.query(
      `SELECT t.id, t.solved, t.ressourceID, t.userID FROM tickets as t INNER JOIN ressources as r ON t.ressourceID = r.id WHERE r.responsableID=${req.user.id};`,
      {
        type: QueryTypes.SELECT,
      }
    );
